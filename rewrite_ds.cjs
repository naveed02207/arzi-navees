...
const getIcon = (name: string, isSelected: boolean) => {
  ...
  switch (name) {
    ...
    default: return <Landmark className={iconClass} />;
  }
};
...
export const DepartmentSelector: React.FC<DepartmentSelectorProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 sm:p-8 transition-all duration-300 border border-gray-100">
      <div className="mb-6">
        <label className={getTextClass("text-sm uppercase tracking-widest font-bold text-emerald-700 block")}>
          {t("lbl_department")}
        </label>
        ...
        {translatedDepts.map((tDept: any) => {
          ...
          return (
            <button
              ...
              className={\`relative h-full flex flex-col justify-center p-6 rounded-xl text-left transition-all duration-300 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 group \${
                isSelected
                  ? "bg-gradient-to-r from-emerald-500 to-teal-500 border-transparent shadow-md"
                  : "bg-white border border-gray-200 shadow-sm hover:-translate-y-1 hover:shadow-xl hover:border-emerald-300"
              }\`}
            >
              {isSelected && (
              ...
              <div className={\`p-3 rounded-xl mb-4 transition-transform group-hover:scale-110 w-max \${
                isSelected ? "bg-white/20" : "bg-emerald-50 border border-emerald-100"
              }\`}>
                {getIcon(dept.iconName, isSelected)}
              </div>
              
              <h2 className={\`text-xl font-bold font-urdu leading-tight w-full mb-2 \${isSelected ? 'text-white' : 'text-emerald-900'}\`}>
                {tDept.name}
              </h2>
...
console.log("Rewrote DepartmentSelector");
